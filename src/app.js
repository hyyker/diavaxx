...
function brand() { return `<a class="brand" href="./" aria-label="Everwell home"><span class="brand-mark">${icon('plus')}</span><span class="brand-text">everwell<span class="brand-dot">.</span></span></a>`; }
function render() {
  const c = counts();
  $('#app').innerHTML = `
...
async function showExport() {
  ...
  $('#save-qr').onclick = () => { const a = document.createElement('a'); a.download = `everwell-qr-${current + 1}-of-${parts.length}.png`; a.href = $('#qr-canvas').toDataURL('image/png'); a.click(); };
  await draw();
}
function showImport() {
...
document.addEventListener('click', async e => {
  ...
  if (action === 'download-code') { if (blocked) { error('Saved data is unreadable. Use Download saved data for recovery.'); return; } download(encode(state), `everwell-backup-${today()}.txt`); }
  if (action === 'raw-backup') { try { download(localStorage.getItem(STORAGE_KEY) || '', `everwell-recovery-${today()}.json`, 'application/json'); } catch { toast('Browser storage cannot be accessed.'); } }
